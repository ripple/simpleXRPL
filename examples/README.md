...
| [02-custodian-connections.ts](./02-custodian-connections.ts) | Constructing each connector (Local, Palisade; Ripple Custody pending) |
...
| File | Shows |
| ---- | ----- |
| [05-rwa-through-ripple-custody.ts](./05-rwa-through-ripple-custody.ts) | Issuing a Real-World Asset as an MPT (XLS-89 metadata) |
...
| [09-cross-custodian-workflow.ts](./09-cross-custodian-workflow.ts) | Sequencing work across two custodians (vertical verbs and `runMultiStep`) |

> **Note:** the Ripple Custody connector is in progress. Sample 05 issues an RWA
> through whatever connector owns the account; the call is identical once Ripple
> Custody is bound — only the account's `from` changes.
